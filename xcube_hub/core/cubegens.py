...
def create_cubegen_object(cubegen_id: str, cfg: AnyDict, info_only: bool = False) -> client.V1Job:
    ...
    cmd = f"source activate xcube && echo CREODIAS: $CREODIAS_ACCESS_KEY_ID && echo \"$(date '+%Y-%m-%d %T') XCUBE GEN START\" && " \
          f"xcube --traceback gen2 -o {res_file} -vv {info_flag} " \
          f"--stores {stores_file} {cfg_file} && echo \"$(date '+%Y-%m-%d %T') XCUBE GEN END\""
