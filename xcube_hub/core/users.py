...
def remove_role_from_user(user_id: str, role_id: str, token: Optional[str] = None):
    token = token or strap_token()
    payload = {
        "roles": [
            f"{role_id}"
    ...
    headers = {'Authorization': f"Bearer {token}"}
    r = requests.delete(f'https://edc.eu.auth0.com/api/v2/users/auth0|{user_id}/roles', json=payload, headers=headers)
