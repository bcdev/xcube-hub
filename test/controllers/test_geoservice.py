...
class TestGeoServer(BaseTestCase):
    def setUp(self):
    ...
    def test_get_collection(self):
        ...
        self._geo._provider = Mock()
        self._geo._provider.get_layer.return_value = {'layer': {'name': 'test'}}

        response = self.client.open('/api/v2/services/geoservice/databases/terrestris/collections/test',
                                    headers=self._headers, method='GET')
        self.assert200(response, 'Response body is : ' + response.data.decode('utf-8'))
        self.assertEqual('test', response.json['layer']['name'])
    ...
    def test_put_collection(self):
        # test whether controller works
        self._geo._provider = Mock()
        self._geo._provider.publish.return_value = {'layer': {'name': 'test'}}
    ...
    def test_delete_collection(self):
        # test whether controller works
        self._geo._provider = Mock()
        self._geo._provider.unpublish.return_value = {'layer': {'name': 'test'}}
