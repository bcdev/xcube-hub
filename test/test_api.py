...
from xcube_gen.api import get_json_request_value, ApiError, ApiResponse
...
class ApiResponseTest(unittest.TestCase):
    def test_success(self):
        self.assertEqual(dict(status='ok'),
                         ApiResponse.success())
        self.assertEqual(dict(status='ok', result=4589),
                         ApiResponse.success(result=4589))

    def test_failure(self):
        self.assertEqual((dict(status='error'), 500),
                         ApiResponse.error())
        self.assertEqual((dict(status='error', message='help!'), 400),
                         ApiResponse.error(error=ValueError('help!'), status_code=400))
